...
export class TicketDatabaseObjectStrategy implements DatabaseObjectStrategy {
    public create(object: object): Promise<object> {
        ...
        return new Promise((resolve, reject) => {
            ConnectionSingleton.getConnection().query("INSERT INTO ticket (movieScreeningId, row)", [ticket.getMovieScreening().getId(), ticket.getRow()], (err, res, fields) => {
                if (err) reject(err);
                else {
    ...
    public update(object: object): Promise<object> {
        ...
        return new Promise((resolve, reject) => {
            ConnectionSingleton.getConnection().query("UPDATE ticket SET row = ? WHERE id = ?", [ticket.getRow(), ticket.getId()], (err, res, fields) => {
                if (err) reject(err);
                else resolve(ticket);
            });
        });
    ...
    public getAll(): Promise<object[]> {
        return new Promise((resolve, reject) => {
            ConnectionSingleton.getConnection().query("SELECT t.id AS id, row, movieScreeningId, datetime, movieId, name, duration, bookingId, cancelled, personId, firstname, lastname FROM ticket t INNER JOIN movieScreening ms ON t.movieScreeningId = ms.id INNER JOIN movie m ON ms.movieId = m.id LEFT JOIN booking b ON t.bookingId = b.id LEFT JOIN person p ON b.personId = p.id", (err, res, fields) => {
                if (err) reject(err);
                else {
